...
void ESP8266_Init(u32 bound)
{
    ...
    RCC_APB2PeriphClockCmd(ESP8266_RST_Pin_Periph_Clock, ENABLE);
...
//	}
	while(time--)
	{
		...
		if(ack1!=0&&ack2!=0)
		{
			if ( ( bool ) strstr ( USART3_RX_BUF, ack1 ) || ( bool ) strstr ( USART3_RX_BUF, ack2 ) )
				return true;						 
		}
		else if( ack1 != 0 )  //strstr(s1,s2);检测s2是否为s1的一部分，是返回该位置，否则返回false，它强制转换为bool类型了
		{
			if ( ( bool ) strstr ( USART3_RX_BUF, ack1 ) )
				return true;
		}
		else
		{
			if ( ( bool ) strstr ( USART3_RX_BUF, ack2 ) )
				return true;	
		}
...
//    printf("%s",ESP8266_Fram_Record_Struct .Data_RX_BUF);
    if(ack1!=0&&ack2!=0)
    {
        return ( ( bool ) strstr ( USART3_RX_BUF, ack1 ) || 
                         ( bool ) strstr ( USART3_RX_BUF, ack2 ) );
    }
    else if( ack1 != 0 )  //strstr(s1,s2);检测s2是否为s1的一部分，是返回该位置，否则返回false，它强制转换为bool类型了
        return ( ( bool ) strstr ( USART3_RX_BUF, ack1 ) );

    else
        return ( ( bool ) strstr ( USART3_RX_BUF, ack2 ) );
